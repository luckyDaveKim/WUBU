...
objStockMst.SetInputValue(0, 'U001')  # 종목코드
objStockMst.SetInputValue(1, ord('2'))  # 요청구분 (1:기간, 2:개수)
objStockMst.SetInputValue(4, 500)  # 요청개수
objStockMst.SetInputValue(5, [0, 1, 2, 3, 4, 5, 8, 9])  # 요청필드 (날짜, 시간, 시가, 고가, 저가, 종가, 거래량, 거래대금)
...
while True:
    ...
    sectors_id = objStockMst.GetHeaderValue(0)[1:]
    ...
    db_manger.insert_sectors_minutely_index(items, sectors_id)
    db_manger.insert_sectors_minutely_volume(items, sectors_id)
    db_manger.insert_sectors_minutely_price(items, sectors_id)

...
df = mk.get_daily_price('삼성전자', '2018-07-01', '2020-03-10')
...
for i in range(1, len(df.close)):
    if df.ema130.values[i-1] < df.ema130.values[i] and \
        ...
        plt.plot(df.number.values[i], df.close.values[i] - 10000, 'r^')
    elif df.ema130.values[i-1] > df.ema130.values[i] and \
        df.slow_d.values[i-1] <= 80 and df.slow_d.values[i] > 80:
        plt.plot(df.number.values[i], df.close.values[i] - 10000, 'bv')
plt.legend(loc='best')
